...
function SpamDetector() {
  ...
  <textarea
    className={`w-full border p-4 pr-12 rounded-2xl focus:outline-none focus:ring-2 resize-none text-sm sm:text-base transition-all shadow-inner leading-relaxed
      ...
      ${
        isDark
          ? `${activeTheme.inputDark} focus:border-blue-500/50 [&::-webkit-scrollbar-thumb]:bg-slate-700`
          : `${activeTheme.input} focus:border-indigo-500/50 [&::-webkit-scrollbar-thumb]:bg-slate-300`
      }`}
    rows="5"
    placeholder={
      type === "url"
        ? "Paste or type the suspicious website link here..."
        : type === "message"
        ? "Type your SMS or chat message here..."
...
{result && (
  <div className={`mt-5 rounded-3xl p-5 shadow-lg border transition-all duration-300 ${
    isDark
    ...
    {/* Message */}
    <div
      className={`p-4 rounded-2xl font-semibold text-center ${getBg()} ${getColor()}`}
    >
      {result === "ham" && "This message appears to be safe."}
      {result === "safe" && "This URL appears to be safe."}
      {result === "spam" && "This content has been detected as spam."}
      {result === "malicious" &&
        "This URL may be malicious or unsafe."}
      {result === "smishing" &&
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <span
            className={`px-3 py-1 rounded-full text-xs font-semibold ${
              Number(confidencePct) >= 80
                ? "bg-red-500/20 text-red-500"
                : Number(confidencePct) >= 50
